...
Overview_portfolio_sector_stack <- function(plotnumber){
  ...
  over1$Fin.Sector <- factor(over1$Fin.Sector, levels=c("Other Sectors","Fossil Fuels", "Automotive","Power"), ordered=TRUE) #"Climate Relevant No 2째 Scenario",
  # over1$Sector.All <- factor(over1$Sector.All, levels=c("Excluded","Other Sectors","Climate Relevant No 2째 Scenario","Climate Relevant w/ 2째 Scenario"), ordered=TRUE)
...
portfolio_sector_stack <- function(plotnumber){
  ...
  over$Sector.All <- ifelse(over$Valid==0, "Excluded", "Climate Relevant w/ 2째Scenario")
  over$Sector.All <- ifelse(over$Sector== "Climate Relevant No 2째Scenario" & over$Valid==1 , "Climate Relevant No 2째Scenario",over$Sector.All)
  over$Sector.All <- ifelse(over$Sector =="Other Sectors" & over$Valid==1, "Other Sectors",over$Sector.All)
  
  over$Sector <- factor(over$Sector, levels=c("Other Sectors","Fossil Fuels", "Automotive","Power"), ordered=TRUE) #,"Climate Relevant No 2째 Scenario",
  over$Sector.All <- factor(over$Sector.All, levels=c("Excluded","Other Sectors","Climate Relevant No 2째Scenario","Climate Relevant w/ 2째Scenario"), ordered=TRUE)
  ...
  over<- over %>%
    complete(Asset.Type=c("Fixed Income","Equity"),
             Sector = c("Other Sectors","Fossil Fuels", "Automotive","Power"), #"Climate Relevant No 2째 Scenario",
  ...
  if (PortName!="MetaPort"){
    plot <- ggplot(data=subset(over, Portfolio.Name==PortName&Valid==1), aes(x=Asset.Type, y=per, fill=Sector)) +
      ...
      scale_fill_manual(name="", labels=c("Other Sectors","Fossil Fuels", "Automotive","Power"), values=c("#deebf7",energy, trans, pow),drop = FALSE) +   #"Climate Relevant No 2째 Scenario", "#90b6e4",
      scale_x_discrete(name="Asset Type") +
...
exposure_summary <- function(plotnumber,ChartType){
  ...
  plot <- ggplot(Portfolio) +
    ...
    ylab("Alignment of Portfolio with 2째Market Benchmark")+
    theme_barcharts()+
...
analysed_summary <- function(plotnumber){
  ...
  # 
  #over$Sector <- factor(over$Sector, levels=c("Other Sectors","Climate Relevant No 2째 Scenario","Fossil Fuels", "Automotive","Power"), ordered=TRUE)
...
sector_techshare <- function(plotnumber,ChartType,SectorToPlot,Plotyear){
  ...
  if (nrow(Combin)>0){
    ...
    Production <- subset(Production, Production$Type != "Market")
      ...
      Production$Type <- factor(Production$Type, levels=c("Portfolio","MetaPortfolio","2캫arket Benchmark"))
...
Graph246_new <- function(plotnumber,ChartType,TechToPlot){
  ...
  if (nrow(ALD_P)>0){
    ...
    outputplot <- ggplot(data = subset(ALD.sc.tall, Technology == TechToPlot & ALD.sc.tall$PortName == PortNames )) +
      ...
      #     subtitle = "Trajectory of Portfolio's Current Plans compared to IEA 2째,4째,6째 Degree Scenarios") +
