...
SectorDataAnalysis <- function(){
  ...
  # over$Sector.All <- ifelse(over$Valid == 0, "Excluded", "Climate Relevant w/ 2째 Scenario")
  # over$Sector.All <- ifelse(over$Sector == "Climate Relevant No 2째 Scenario" & over$Valid == 1 , "Climate Relevant No 2째 Scenario",over$Sector.All)
...
Overview_portfolio_sector_stack <- function(plotnumber){
  ...
  # over$Sector <-ifelse (over$Subgroup %in% OilGasCoal,"Fossil Fuels",over$Sector)
  # over$Sector <-ifelse (over$Subgroup %in% Auto,"Automotive",over$Sector)
  # over$Sector <-ifelse (over$Subgroup %in% Futuresecs,"Climate Relevant No 2째 Scenario",over$Sector)
  # 
  ...
  #                Sector=c("Other Sectors","Climate Relevant No 2째 Scenario","Fossil Fuels", "Automotive","Power"),
...
Oilshare <- function(plotnumber, companiestoprint, ChartType){
  ...
  if (nrow(OilCompanies) > 0) {
    ...
    PortPlot <- ggplot(data=OilCompanies, aes(x=reorder(Name,PortWeightEQYlvl), y=OilShare,
      ...
      annotation_custom(
        grob = textGrob(label = "Weight", hjust =-0.1,gp=gpar(fontsize=12,col=textcolor)),
...
Graph246_new <- function(plotnumber,ChartType,TechToPlot){
  ...
  filternames <- c("MetaPort","Listed Market", "Bond Universe",PortName)
  ### EQUITY PRODUCTION
  if (ChartType =="EQ") {
  ...
  outputplot <- ggplot(data = subset(ALD.sc.tall, Technology == TechToPlot & PortName ==PortName )) +
