...
Overview_portfolio_sector_stack <- function(plotnumber){
  ...
  over1$Sector <- factor(over1$Sector, levels=c("Other Sectors","Fossil Fuels", "Automotive","Power"), ordered=TRUE) #"Climate Relevant No 2째 Scenario",
...
portfolio_sector_stack <- function(plotnumber){
  ...
  over$Sector.All <- factor(over$Sector.All, levels=c("Excluded","Other Sectors","Climate Relevant No 2째 Scenario","Climate Relevant w/ 2째 Scenario"), ordered=TRUE)
...
exposure_summary <- function(plotnumber,ChartType){
  ...
  plot <- ggplot(Portfolio) +
    ...
    ylab("Alignment of Portfolio with 2째 Market Benchmark")+
    theme_barcharts()+
...
company_og_buildout <- function(plotnumber, companiestoprint, ChartType){
  ...
  if (nrow(comp)>0){
    ...
    Label <- "No Companies with either Oil or Gas exposure in your Portfolio"
...
company_techshare <- function(plotnumber, companiestoprint, ChartType, SectorToPlot){
  ...
  if (nrow(combin) > 0) {
    ...
    bar_size = 4/15
    height <- min(11,n_distinct(AllData$Name))*bar_size
...
sector_techshare <- function(plotnumber,ChartType,SectorToPlot,Plotyear){
  ...
  Batch2$Type <-"2째 Market Benchmark"
  ...
  if(nrow(Production)>0){
    ...
    Production$Type <- factor(Production$Type, levels=c("Portfolio","MetaPortfolio","2째 Market Benchmark"))
...
Oilshare <- function(plotnumber, companiestoprint, ChartType){
  ...
  if (nrow(OilCompProdSS)==0){
    ...
    ggsave(filename=paste0(plotnumber,"_",PortfolioName,"_",ChartType,'_OilShare.png', sep=""),
           bg="transparent",height=1.3,width=10,dpi=ppi)
...
carboninout <- function(plotnumber, companiestoprint, ChartType){
  ...
  if (nrow(CompProdSS)==0){
    Label <- "No Companies data in your Equity Portfolio"
    outputplot <- no_chart(Label)+
      theme(panel.background = element_rect(fill = "white", colour = "grey50"))
